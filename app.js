...
function loadState() {
    try {
        ...
        if (!window.hasRunDataRepair) {
            ...
            (typeof categoriesData !== 'undefined' ? categoriesData : []).forEach(c => c.problems.forEach(p => roadmapIds.add(String(p.id))));
            (typeof customSections !== 'undefined' ? customSections : []).forEach(s => (s.problems || []).forEach(p => roadmapIds.add(String(p.id))));
